...
class BPRSLIM(BaseRecommender):
    ...
    def fit(self, urm, target_users, target_items, dataset):
        ...
        if self.urm_ext is None:
            # call cbf
            pass

        if self.urm_ext is not None:
            self._computeEligibleUsers(self.urm_ext)
        if icm is not None:
        ...
        self.cythonEpoch = SLIM_BPR_Cython_Epoch(self.urm_ext,
                                                 icm,
                                                 self.eligibleUsers,
