    ...
    def fit(self, urm, dataset, tg_playlist, tg_tracks, no_components=500, n_epochs=5, item_alpha=1e-5, l_rate=1e-3):
        ...
        if self.ucm_t is None:
            ...
            self.ucm_t = hstack((id_playlist,
                                 ucm.transpose() * 50)).tocsr().astype(np.float32)
...
if __name__ == '__main__':
    ...
    ds.set_track_attr_weights_2(1, 1, 0.1, 0.1, 1, num_rating_weight=1, inferred_album=1, inferred_duration=0.1, inferred_playcount=0.1)
