...
from src.SLIM_BPR.Recommender_utils import similarityMatrixTopK, check_matrix
import numpy as np
cimport numpy as np
import time
import sys
...
cdef class SLIM_BPR_Cython_Epoch:
    ...
    def epochIteration_Cython(self):
        ...
        while index < self.n_items:
            ...
            if self.sparse_weights:
                self.S_sparse.add_value(index, index, -self.S_sparse.get_value(index, index), False)
            else:
