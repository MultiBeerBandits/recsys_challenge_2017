...
def main():
    ...
    n_values = 5
    artist_weights = np.linspace(0.1, 1, n_values)
    album_weights = np.linspace(0.1, 1, n_values)
    duration_weights = np.linspace(0.1, 1, n_values)
    playcount_weights = np.linspace(0.1, 1, n_values)
            ...
            for dur_v in duration_weights:
                for pl_v in playcount_weights:
                    ...
                    with open('cbf_cv_info.log', mode='a') as out_file:
                        ...
                        to_write = 'MAP@5=' + str(map_at_five) + 'Obtained with ' + to_write + ' written in ' + csv_name
