...
def mix_cluster(models, params, tg_playlist, urm=None, icm=None, ds=None):

    urm_red = urm[[ds.get_playlist_index_from_id(x) for x in tg_playlist]]
    ucm = ds.build_ucm()[:, [ds.get_playlist_index_from_id(x) for x in tg_playlist]]

    # user cluster contains only cluster of target users
    user_cluster = build_user_cluster(urm_red, icm, ucm, int(len(params))) #/ 3))
    ...
    for i in range(len(models)):
        ...
        weights = [params[user_cluster(x)+ i*len(user_cluster)] for x in range(len(user_cluster))]
...
def fit(params):
    ...
    ials.fit(tg_playlist, tg_tracks, ds)
    ...
    ev.evaluate_fold(recs_ials)
    # Mix them all
    models = [xbf, cbf, ubf, ials]
    ...
    # multiply both
    R_hat_mult = max_normalize(xbf.R_hat).multiply(max_normalize(cbf.R_hat)).multiply(max_normalize(ubf.R_hat).multiply(max_normalize(ials.R_hat)))
